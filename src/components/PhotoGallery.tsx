...
const PhotoGallery = ({ uploadedPhotos = [] }: PhotoGalleryProps) => {
  ...
  const [panOffset, setPanOffset] = useState({ x: -22, y: -80 }); // Center on sample 2 image. more negative x to left
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      e.preventDefault();
      const zoomSpeed = 0.1;
      const newZoom = Math.max(1.1, Math.min(1.3, zoom + (e.deltaY > 0 ? -zoomSpeed : zoomSpeed))); //zoom range
      setZoom(newZoom);
    };
...
//line 378 adjust view port size
  return (
    <>
      ...
      >
        <div className="relative" style={{ width: "160vw", height: "160vh" }}> 
          <div 
