...
const PhotoGallery = ({ uploadedPhotos = [] }: PhotoGalleryProps) => {
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      e.preventDefault();
      const zoomSpeed = 0.1;
      const newZoom = Math.max(1, Math.min(2, zoom + (e.deltaY > 0 ? -zoomSpeed : zoomSpeed)));
      setZoom(newZoom);
    };
  ...
  return (
    <>
      ...
      >
        <div className="relative" style={{ width: "calc(200vw + 10000px)", height: "calc(200vh + 10000px)" }}>
          <div 
